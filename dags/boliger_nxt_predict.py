"""Boliger NXT 장시작 예측 트리거 DAG.

NXT 장시작(KST 08:00)에 실행하여:
1. 전일 추출된 candidate pickle 로드 (NXT 후보만)
...
def load_and_predict(**context):
    """NXT 후보 pickle 로드 → 시가 조회 → gap 필터 → 완성 pickle + Kafka."""
    import numpy as np
    ...
    nxt_codes = data.get("nxt_codes", [])
    candidates = data.get("candidates", {})
    config = data.get("config", DEFAULT_CONFIG)

    print(f"Loaded {len(nxt_codes)} NXT candidates from {pickle_path}")

    if not nxt_codes:
        raise AirflowSkipException("No NXT candidates.")
    ...
    print(f"  Got prices for {len(price_map)}/{len(nxt_codes)} stocks")
    ...
    ready_path = output_dir / f"nxt_ready_{today_str}.pkl"
...
with DAG(
    dag_id="boliger_nxt_predict",
    default_args=default_args,
    description="Boliger NXT 장시작 예측 트리거",
    ...
    catchup=False,
    tags=["boliger", "nxt", "predict", "kafka"],
) as dag:
    ...
    wait = PythonOperator(
        ...
        python_callable=wait_for_nxt_open,
    )
    ...
    publish = KafkaPublishOperator(
        ...
        message_key="boliger_nxt_{{ execution_date.strftime('%Y%m%d') }}",
    )
