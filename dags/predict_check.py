...
daily_nxt_data DAG 완료 후 실행되어
GapPredictions 테이블의 실제 결과를 계산하여 업데이트합니다.

실행 시나리오:
1. daily_nxt_data DAG 완료 대기
...
with DAG(
    ...
    description='예측 결과 업데이트 - daily_nxt_data 완료 후 실행',
    schedule_interval='31 11 * * 1-5',  # 평일 20:31 KST (daily_nxt_data 이후)
    ...
    # daily_nxt_data DAG 완료 대기
    wait_daily_data = ExternalTaskSensor(
        task_id='wait_daily_nxt_data',
        external_dag_id='daily_nxt_data',
